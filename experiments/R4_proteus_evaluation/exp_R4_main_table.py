...
def make_arf(seed, c):
    # S7-ter/LOT B: warning_detector unified onto the drift detector. It was left unset, which River
    # 0.23.0 resolves to ADWIN(delta=0.01, clock=32) -- two parameters away from the drift detector,
    # not one. make_srp below already pinned both at (0.002, c) and is the pattern copied here.
...
def build_caption(sign_df):
    ...
    return (r"Concept Drift detection on heteroscedastic ProteuS streams with the static RF resolution, "
            ...
            r"resamples of the 30 SEED indices, paired across arms, not of the individual runs); "
            r"$\infty$ denotes detection failure. Significance is assessed at the seed level (the unit "
