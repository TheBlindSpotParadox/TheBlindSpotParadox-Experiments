...
\newcommand{\LearnShare}{99.3}               % s6\_causal.json, erasure\_share\_post\_fork
                                             % .a\_pos\_common.pooled.median\_share\_learn = 0.99337
                                             % (E\_learn 819.2 / E\_total 826.7, n = 1979)
...
% ══════════════════════════════════════════════════════════════════
\begin{abstract}
  Concept drift monitoring on data streams typically pairs an adaptive classifier with an external drift detector. We show that this standard architecture harbors a fundamental race condition: adaptive classifiers such as Adaptive Random Forests (ARF) absorb distributional changes before CUSUM-family monitors accumulate sufficient evidence, erasing the drift signal over the exploitable transient. Through direct instrumentation of the ARF's internal tree-swap dynamics we separate the roles rather than reduce the phenomenon to one cause: internal ADWIN hyper-reactivity fixes the \emph{onset} of adaptation, ordinary incremental learning of the surviving trees performs $\LearnShare\%$ of the \emph{erasure}, and the replacements after the first act as a terminal \emph{lock} on the thresholded decision. The phenomenon has two regime-dependent faces: starvation (recall collapse) on stationary streams and false-alarm flooding (precision collapse) on non-stationary streams. A Starvation Effect renders the transient signal too brief for CUSUM accumulation, producing missed detections or alarms that fire only after adaptation; an ensemble-level Hydra Effect accelerates this adaptation by at least 4.1 to 8.0 times over a single tree. We recast decoupling as a reliability condition on stopping times and give a deterministic detection certificate calibrated on the evidence ceiling the monitor can actually reach. The certificate exposes a detectability floor on drift magnitude: below it, bounding false alarms under heteroscedastic volatility requires a threshold above the ceiling and the admissible calibration set is empty; above it the set is non-empty and a calibration exists. Synchronised instrumentation of $8{,}000$ runs measures the ceiling directly and locates the floor at $\Delta e_c = \DeCrit$ ($95\%$ bootstrap CI $\DeCritCI$, $n = 300$), with operational threshold $\lambda_{\mathrm{op}} = \LambdaOpNarrow$ $\LambdaOpNarrowCI$ over the envelope $[\EnvLo, \EnvHi]$. The result is a measured bound, not an impossibility. A distribution-based KSWIN monitor avoids the constraint on the settings we test---a regime-restricted observation rather than an immunity---recovering reliable detection at no predictive cost. Validation spans instrumented Bernoulli race experiments across a wide range of drift magnitudes, heteroscedastic ARMA--GARCH streams (ProteuS), and six real-world streams (BAF, INSECTS); standard benchmarks (SEA, Hyperplane) operate below the floor and mask the phenomenon.
\end{abstract}
...
When the ARF adapts \emph{faster} than such a monitor accumulates evidence, the drift signal is erased over the exploitable transient---the \textbf{blind spot phenomenon}.
...
\noindent\textbf{Hoeffding Trees (HT)}~\cite{gama_survey_2014} are incremental decision trees, \emph{non-adaptive} once converged. \textbf{Adaptive Random Forests (ARF)}~\cite{gomes_arf_2017} maintain $M$ HTs, each paired with an internal ADWIN: upon an internal drift warning the ARF pre-trains a background tree and, upon confirmation, instantly replaces the active tree. This adaptive \emph{tree-swap} mechanism is central to the blind spot phenomenon.
...
We formalize the blind spot phenomenon through direct instrumentation of the Adaptive Random Forest (ARF) in River 0.23.0~\cite{montiel_river_2021}.
...
We formalized the \textbf{blind spot phenomenon}: adaptive classifiers systematically defeat external \emph{cumulative-evidence} drift detectors by absorbing distributional changes before the accumulator reaches its alarm threshold. We separated its roles: internal ADWIN hyper-reactivity fixes the onset, incremental learning of the surviving trees performs $\LearnShare\%$ of the erasure, and the replacements after the first act as a terminal lock on the thresholded decision---the \emph{Hydra Effect} accelerates onset rather than driving erasure. We derived the critical size $M_{\mathrm{crit}}$ beyond which cumulative monitors structurally fail. Windowed and distributional monitors are spared by construction.
