...
\textbf{The Fundamental Tension.} On heteroscedastic streams, guaranteeing reliable detection caps the CUSUM threshold at $\lambda_{\mathrm{op}} \le 12.44$ when the calibration sweep uses $T_{\mathrm{drift}} = 2000$, and at $\lambda_{\mathrm{op}} \le 10.5$ once its warm-up is matched to the ProteuS streams ($T_{\mathrm{drift}} = 4000$), whereas controlling GARCH-induced false alarms requires $\lambda \ge 15$. We state the parity gap rather than hide it: the $\lambda \ge 15$ member is the fixed ProteuS operating threshold, armed from $t{=}0$ with no warm-up and on a different stream family, so the two members are not measured under one protocol. The conclusion survives the mismatch---the admissible set is empty under either calibration ($\{\lambda \le 12.44\} \cap \{\lambda \ge 15\} = \emptyset$) and the gap \emph{widens} from $2.6$ to $4.5$ at warm-up parity---and forces a choice among three architectural fixes:
