...
| I7  | ~~`articleA_biblio_v2.bib` = v64 ++ additions~~ **CLOS par A6.** Pas de troisième fichier : les 17 additions S5 sont fusionnées dans `articleA_biblio_v64.bib`, qui devient l'unique bibliographie, et `articleA_biblio_v2_additions.bib` est supprimé. Les 4 entrées réclamées par `notation_map` (`kingman_1970`, `lai_1998`, `siegmund_1985`, `tsybakov_2009`) y sont ajoutées. 54 entrées, clés uniques, recouvrement nul avant fusion. Opération **prouvée neutre** sur le document compilé : le `.bbl` est byte-identique avant et après, 33 bibitems — BibTeX n'émet que les entrées citées et il n'y a aucun `\nocite`. |                                                                                                                                                   | Intégration     | Action manuelle          |
