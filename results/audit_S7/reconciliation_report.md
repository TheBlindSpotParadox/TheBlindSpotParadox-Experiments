...
| 4bis | bit-for-bit parquet/CSV identity of R6 and R9 after the SSOT refactor, against `artifacts_sha256_pre_ssot.txt` | **`R6_hat_instrumented.parquet: OK`, `results_instrumented_A_ADWIN_HAT.csv: OK`, `exp_R9_mcrit_comparison.csv: OK`.** Full freeze re-verified: **all 34 artifacts unchanged.** This is the empirical proof of the CONFIG §3 two-draws-per-step invariant and supersedes item 2 as the strongest available evidence. **CORRECTION (stream S7-ter, action D-3):** that sentence was true on 2026-09-06 and is false since. Actions A1 (`8fb0875`) and A2 (`70ca9ae`), both 2026-09-12, moved five of the 34 artifacts; `sha256sum -c` now returns **29 OK / 5 FAILED**, each of the five declared with its motive and measured effect in `_baseline/authorized_deviations.txt`. A reader of this report alone was misled about the live freeze state |
