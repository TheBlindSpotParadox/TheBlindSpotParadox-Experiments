...
DE_MONOTONE_FROM = 0.26           # under restored U0, monotonicity holds across the entire band;
                                  # under unified U1 it dipped once at 0.29
DE_MIN = 0.02
...
def test_starvation_dominates_the_adaptive_arm_at_the_top_of_the_grid():
    """ARM-DEPENDENT. Restored U0 published baseline: 100.0 % (unified U1 ablation: 80.0 %).

    Under River defaults (delta=0.01, clock=32), background trees learn and the resulting
    faster adaptation drives missed detections to 100 % at the top of the grid."""
    df = _metrics()
    # Baseline U0 publiée: 100.0 % (sous U1: 80.0 %)
    assert _at(df, "ARF", DE_MAX, "miss_mean") == 100.0, "ARF miss rate at Delta_e = 0.50 moved again"
...
def test_missed_detections_climb_monotonically_across_the_blind_spot_band():
    ...
    is restored U0. Under U0, the band runs monotonically 1 % -> 100 % from DE_BLIND_SPOT_ONSET.
    (Under unified U1 it ran 4 % -> 80 % with monotonicity starting at 0.29)."""
    df = _metrics()
    band = (df[(df.pipeline == "ARF") & (df.delta_e >= DE_BLIND_SPOT_ONSET)]
            .sort_values("delta_e"))
    # U0 publiée: 1.0 % (sous U1: 4.0 %)
    assert band.miss_mean.iloc[0] == pytest.approx(1.0, abs=0.5), (
        f"miss rate at band onset is {band.miss_mean.iloc[0]}, expected 1% on U0")
    # U0 publiée: 100.0 % (sous U1: 80.0 %)
    assert band.miss_mean.iloc[-1] == 100.0
    mono = band[band.delta_e >= DE_BLIND_SPOT_ONSET]
...
def test_monitoring_fidelity_costs_23_4_accuracy_points():
    ...
    # 23.41 pp sur U0, 22.99 sur U1 : le 23.4 du manuscrit reproduit U0 à 0.01 pp près.
    assert gap_pp == pytest.approx(23.41, abs=0.5), (
        f"post-drift accuracy gap ARF - RF at Delta_e = 0.50 is {gap_pp:.2f} pp")
    assert _at(df, "ARF", DE_MAX, "acc_mean") == pytest.approx(0.984, abs=0.005)
